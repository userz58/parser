...
class ProductsBlocksExtractor implements ExtractorInterface
{
    const BASE_HREF = 'https://rothenberger.ru';

    protected string $label = 'Товары';

    protected string $selector = '#right_block_ajax > .inner_wrapper > .ajax_load > .js_wrapper_items .items_wrapper > .catalog_block > .item_block';
    ...
    public function extract(Crawler $crawler): array
    {
        ...
        $productsList = $crawler->filter($this->selector)->each(function (Crawler $node, $i) {
            ...
            return [
                ...
                'Артикул' => $node->filter('.article_block')->attr('data-value'),
