...
#[AsCommand( name: 'parser:voll', description: 'парсинг инструмента Voll.Ru' )]
class VollRuParserCommand extends Command
{
    /** @var string */
    const FILESYSTEM_DIR = 'voll-ru';

    /** @var string */
    protected $filepath = '/srv/app/public/voll-ru.xls';
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $categories = [];
        $catalogCategories = $this->parseIndexCategories('https://voll.ru/product/');
    ...
    protected function parseIndexCategories(string $url)
    {
        ...
        $links = $crawler->filter('.section-content-wrapper .catalog .items .item a')->each(function (Crawler $node, $i) {
    ...
    protected function parseProductsList(string $url, string $name = '???')
    {
        ...
        while (true === $hasNextPage) {
            ...
            $productsList = $crawler->filter('.catalog_page div[itemprop=itemListElement]')->each(function (Crawler $node, $i) {
                return [
                    'url' => sprintf('%s%s', self::BASE_HREF, $node->filter('.title a')->attr('href')),
                    'sku' => $node->filter('.arts .article span')->text(),
                    'name' => trim(preg_replace('/\s+/', ' ', $node->filter('.title a')->text())),
                ];
            });
            ...
            if ($crawler->filter('.pagination')->count() > 0) {
                $nextPageLink = $crawler->filter('.pagination .next a');
    ...
    protected function parseProductPage(string $url, array $data = [])
    {
        ...
        $data['images'] = $crawler->filter('.galery .slides .item a.fancybox')->each(function (Crawler $node, $i) {
            return sprintf('%s%s', self::BASE_HREF, $node->attr('href'));
        });
        ...
        $characteristics = $crawler->filter('.section-content-wrapper #props .char-wrapp');
        if ($characteristics->count() > 0) {
            $characteristicsHtml = preg_replace("/<([a-z][a-z0-9]*)[^>]*?(\/?)>/si", '<$1$2>', $characteristics->html());
            $characteristicsHtml = str_ireplace([chr(9), chr(10)], '', $characteristicsHtml);
        ...
        $modifications = $crawler->filter('.section-content-wrapper #modifications meta[itemprop=name]')->each(function (Crawler $node, $i) {
            return trim(preg_replace('/\s+/', ' ', $node->attr('content')));
        });

        $accessories = $crawler->filter('.section-content-wrapper #accessories meta[itemprop=name]')->each(function (Crawler $node, $i) {
            return trim(preg_replace('/\s+/', ' ', $node->attr('content')));
        });

        $parts = $crawler->filter('.section-content-wrapper #dopparts meta[itemprop=name]')->each(function (Crawler $node, $i) {
            return trim(preg_replace('/\s+/', ' ', $node->attr('content')));
        });
        ...
        $breadcrumbs = $crawler->filter('.breadcrumb a[itemprop=item]')->each(function (Crawler $node, $i) {
            return [
